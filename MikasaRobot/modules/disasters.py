...
from MikasaRobot.modules.helper_funcs.chat_status import (
    dev_plus,
    sudo_plus,
    whitelist_plus,
)
from MikasaRobot.modules.helper_funcs.extraction import extract_user
from MikasaRobot.modules.log_channel import gloggable
...
def addsudo(update: Update, context: CallbackContext) -> str:
    ...
    if user_id in DRAGONS:
        message.reply_text("This Person is already My Captain")
        return ""

    if user_id in DEMONS:
        rt += "uff Now My Comrade become Captain."
        data["supports"].remove(user_id)
        DEMONS.remove(user_id)

    if user_id in WOLVES:
        rt += "Really! why you making My frnd to Comrade."
...
def addsupport(
    ...
    if user_id in DEMONS:
        message.reply_text("This person is already My comrade.")
        return ""
    ...
    update.effective_message.reply_text(
        rt + f"\n{user_member.first_name} was Made my comrade!",
    )
...
def addtiger(update: Update, context: CallbackContext) -> str:
    ...
    if user_id in DRAGONS:
        rt +=  "Lmao,Captain powers taken."
        data["sudos"].remove(user_id)
        DRAGONS.remove(user_id)

    if user_id in DEMONS:
        rt += "Bad Comrade make him best frnd."
        data["supports"].remove(user_id)
        DEMONS.remove(user_id)
...
def removesudo(update: Update, context: CallbackContext) -> str:
    ...
    if user_id in DRAGONS:
        message.reply_text("Mom told me to stay away from strangers")
        DRAGONS.remove(user_id)
        data["sudos"].remove(user_id)
...
SUDO_HANDLER = CommandHandler(("addsudo", "addcaptain"), addsudo, run_async=True)
SUPPORT_HANDLER = CommandHandler(("addsupport", "addcomrade"), addsupport, run_async=True)
